...
public class LoginSyncDataStatusObserver {
    private static String TAG = LogUtils.makeLogTag(LoginSyncDataStatusObserver.class.getSimpleName());
    private LoginSyncStatus loginSyncStatus = LoginSyncStatus.NO_BEGIN;
    ...
    public void registerLoginSyncDataStatus(boolean register) {
        NIMClient.getService(AuthServiceObserver.class).observeLoginSyncDataStatus(new Observer<LoginSyncStatus>() {
            ...
            public void onEvent(LoginSyncStatus status) {
                loginSyncStatus = status;
                if (status == LoginSyncStatus.BEGIN_SYNC) {
                    LogUtils.LOGI(TAG, "login sync data begin");
                } else if (status == LoginSyncStatus.SYNC_COMPLETED) {
                    LogUtils.LOGI(TAG, "login sync data completed");
    ...
    public boolean observeSyncDataCompletedEvent(Observer<Void> observer) {
        ...
        // 超时定时器
        if (uiHandler == null) {
            uiHandler = new Handler(AppUtil.getContext().getMainLooper());
        }
    ...
    private void onLoginSyncDataCompleted(boolean timeout) {
        LogUtils.LOGI(TAG, "onLoginSyncDataCompleted, timeout=" + timeout);
